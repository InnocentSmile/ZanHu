...
class NewsViewsTest(TestCase):
    ...
    def test_post_news(self):
        ...
        response = self.client.post(
            reverse("news:post_news"), {"post": "我是彭志浪"},
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",  # 表示发送Ajax Reques请求
        )
        assert response.status_code == 200
        assert News.objects.count() == initial_count + 1
    ...
    def test_get_thread(self):
        ...
        response = self.client.get(
            reverse("news:get_thread"), {"news": self.first_news.pk},  # 让user01给自己的动态点赞
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",  # 表示发送Ajax Reques请求
        )
        assert response.status_code == 200
